...
preBalanceTxIO pabConf ownPkh tx =
  runEitherT $
    do
      ...
      let scriptOuts = fmap (Tx.toTxOut . fromScriptOutput) $ unBalancedTxUtxoIndex tx
          utxoIndex = fmap Tx.toTxOut utxos <> scriptOuts
      ...
      Files.writeAllForTx pabConf txWithoutFees
